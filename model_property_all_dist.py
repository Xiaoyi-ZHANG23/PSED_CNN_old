...
class LearningMethod:
    def __init__(self, network, optimizer, criterion, dist, logger=None, rank=0):
        self.net = network
    ...
    def train(
        ...
        for e in range(epochs):
            ...
            # Training phase
            for i, (X_train, y_train, extras_train) in enumerate(train_loader):
                self.net.train()
            ...
            epoch_train_loss_tensor = torch.tensor(epoch_train_loss / num_train_samples, device=device)
            if self.logger:
                self.logger.info(f"Epoch {e} train loss: {epoch_train_loss_tensor.item()}")
            self.dist.all_reduce(epoch_train_loss_tensor, op=self.dist.ReduceOp.SUM)
            if self.logger:
                self.logger.info(f"Epoch {e} train loss after reduction: {epoch_train_loss_tensor.item()}")
            epoch_train_loss_avg = epoch_train_loss_tensor.item() / self.dist.get_world_size()
            ...
            if epoch_val_loss < best_val_loss:
                ...
                best_model_state = self.net.module.state_dict()
                no_improve_epochs = 0
            else:
                no_improve_epochs += 1
            ...
            if no_improve_epochs >= self.early_stopping_patience:
                ...
                self.net.module.load_state_dict(best_model_state)
                break
