...
# -----------------------------------------------------------------
from model_property_all_dist import (
    RetNet,
    LearningMethod,
    CustomDataset,
    init_weights,
...
# -----------------------------------------------------------------
hyper_params = {
    'batch_size':   128, # increase batch size according number of GPUs used
    'num_epochs':   1000,
    'eval_freq':    40,
    'learning_rate':1e-4,
    'weight_decay': 1e-4,
...
model_name = f'Mix{pressure_str}_{num_grids}_grids_{grid_type}_{target_col}_extrafea_{len(property_cols)}_no_64_{timestamp}'
...
def setup():
    ...
    # Ensure LOCAL_RANK is set
    if "LOCAL_RANK" not in os.environ:
        raise RuntimeError("LOCAL_RANK environment variable is not set.")

    rank = int(os.environ["RANK"])  # Global rank across all processes
    nproc = int(os.environ["WORLD_SIZE"])  # Total number of processes
    local_rank = int(os.environ["LOCAL_RANK"])  # Local rank on this node

    # Assign the GPU based on LOCAL_RANK
    torch.cuda.set_device(local_rank)

    print(f"Rank {rank}/{nproc}: Initializing process group...")

    # Initialize process group (pass explicit rank and nproc)
...
def main():
    ...
    dist = setup()
    print(f"Start running basic DDP example on rank {dist.get_rank()}.")
    rank = dist.get_rank()
    ...
    # Load data
    if logger:
        logger.info(f"Loading data from {load_dir}")
    ...
    def get_extras_list(dir_path):
        ...
        return df_all.loc[names, property_cols].values.astype(np.float32)
    ...
    if logger:
        logger.info(f"Build distributed sampler ")
    train_sampler = DistributedSampler(train_dataset, num_replicas=nproc, rank=rank)
    val_sampler = DistributedSampler(val_dataset, num_replicas=nproc, rank=rank)

    num_worker = 2
    
    train_loader = DataLoader(train_dataset, batch_size=hyper_params['batch_size'], sampler=train_sampler, num_workers=num_worker, pin_memory=True, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=hyper_params['batch_size'], sampler=val_sampler, num_workers=num_worker, pin_memory=True)
    
    

    # -----------------------------------------------------------------
    # 6) Instantiate RetNet with extra_dim=1
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    if rank == 0:
        test_loader = DataLoader(test_dataset, batch_size=hyper_params['batch_size'], num_workers=num_worker, pin_memory=True)
        model.net.eval()
        predictions = []
        true_values = []
        ...
        plt.grid(True)
        parity_plot_path = os.path.join(output_dir, f'parity_{model_name}.png')
